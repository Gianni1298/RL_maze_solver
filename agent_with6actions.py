...
class Agent:
    ...
    def get_next_action(self, state):
        if self.num_steps_taken % self.episode_length == 0 and self.K % 8 == 0 and self.K>10:
        ...
        action_probability = np.ones(5) * (self.epsilon / 5)
        action_probability[action] += (1 - self.epsilon)
        #action_probability[2] = 0

        action = random.choices([0,1,2,3,4], action_probability)
        action = action[0]
    ...
    def discrete_to_continuous(self, discrete_action):
        ...
        if discrete_action == 3: #Move up
            continuous_action = np.array([0.02, 0.02], dtype=np.float32)
        if discrete_action == 4: #Move up
            continuous_action = np.array([0.02, -0.02], dtype=np.float32)
        return continuous_action
    ...
    def set_next_state_and_distance(self, next_state, distance_to_goal):
        ...
        #norm_steps_taken = (((self.num_steps_taken-1) % self.episode_length)-100)/40000


        reward = (1 - distance_to_goal) * hit_wall
...
class DQN:
    ...
    def __init__(self):
        ...
        self.q_network = Network(input_dimension=2, output_dimension=5)
    ...
    def calculate_Bellman_reward(self, minibatch):
        ...
        gamma = torch.ones(len(minibatch)) * 0.99
