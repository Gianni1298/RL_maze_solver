...
class Agent:
    ...
    def __init__(self):
        ...
        self.evaluate_epsilon_greedy = False
        self.num_steps_taken_greedy_policy = 1000
        self.num_steps_taken_greedy_policy_previous = 1000
        self.exploit_epsilon = False
        self.improve_epsilon = False
        self.increase_epsilon = False
        self.decrease_slowly = False
        ...
        if self.num_steps_taken % self.episode_length == 0:
            ...
            if self.K > 30 and self.K % 5  == 0:
                if self.episode_length > 300:
                    self.episode_length = self.episode_length - 50
                    print("The episode_length is:", self.episode_length)


            if self.exploit_epsilon == True:
                if self.epsilon > 0.11:
                    self.epsilon = self.epsilon - 0.025
                    self.previous_epsilon = self.epsilon
            if self.improve_epsilon == True:
                if self.epsilon > 0.11 and self.K % 5 == 0:
                    self.epsilon = self.epsilon - 0.05
                    self.previous_epsilon = self.epsilon
            if self.increase_epsilon == True:
                self.epsilon = self.epsilon + 0.01
                self.previous_epsilon = self.epsilon
            if self.decrease_slowly == True:
                if self.epsilon > 0.32 and self.K % 10 == 0:
                    self.epsilon = self.epsilon - 0.05
                    self.previous_epsilon = self.epsilon
                elif self.epsilon < 0.32:
                    self.decrease_slowly = False
                    self.increase_epsilon = True

            print("The current epsilon is:", self.epsilon)
            # print("self_exploit is:", self.exploit_epsilon)
            # print("improve_epsilon is:", self.improve_epsilon)
            # print("increase_epsilon:", self.increase_epsilon)
            # print("decrease_slowly is:", self.decrease_slowly)
            return True
        else:
            return False
    ...
    def get_next_action(self, state):
        if self.num_steps_taken % self.episode_length == 0:
            if self.K > 10 and self.K % 10 == 0:
                self.evaluate_epsilon_greedy = True
                self.previous_epsilon = self.epsilon
                print("I am evaluating greedy policy")

        if self.evaluate_epsilon_greedy == True or self.stop_training == True:
            self.epsilon = 0
    ...
    def set_next_state_and_distance(self, next_state, distance_to_goal):
        ...
        if self.evaluate_epsilon_greedy == True:
            if distance_to_goal < 0.03:
                print("The number of steps taken with the greedy policy are:", (self.num_steps_taken)%self.episode_length)
                self.num_steps_taken_greedy_policy_previous = self.num_steps_taken_greedy_policy
                self.num_steps_taken_greedy_policy = (self.num_steps_taken)%self.episode_length
                self.evaluate_epsilon_greedy = False
                self.epsilon = self.previous_epsilon

                if self.num_steps_taken_greedy_policy < 100:
                    self.exploit_epsilon = True
                    self.improve_epsilon = False
                    self.increase_epsilon = False
                    self.decrease_slowly = False
                if 300 > self.num_steps_taken_greedy_policy > 100:
                    self.exploit_epsilon = False
                    self.improve_epsilon = True
                    self.increase_epsilon = False
                    self.decrease_slowly = False
                if self.num_steps_taken_greedy_policy > 80 and self.num_steps_taken_greedy_policy > self.num_steps_taken_greedy_policy_previous:
                    self.increase_epsilon = True
                    self.exploit_epsilon = False
                    self.improve_epsilon = False
                    self.decrease_slowly = False
                if self.num_steps_taken_greedy_policy > 300:
                    self.increase_epsilon = False
                    self.exploit_epsilon = False
                    self.improve_epsilon = False
                    self.decrease_slowly = True

                if self.num_steps_taken_greedy_policy < 100:
                    self.stop_training = True

            if self.num_steps_taken%self.episode_length == 0 and distance_to_goal > 0.03:
                print("Did not reach goal with greedy policy")
                self.evaluate_epsilon_greedy = False
                self.epsilon = self.previous_epsilon
                self.decrease_slowly = True
                self.exploit_epsilon = False
    ...
    def train_agent(self, transition):
        ...
        if len(self.replay_buffer.buffer) > self.minibatch_size and self.K > 15 and self.stop_training == False:
...
class ReplayBuffer:
    ...
    weights = deque(maxlen=10000)
    probabilities = deque(maxlen=10000)
    epsilon = 0.001
    alpha = 0.6
...
class DQN:
    ...
    def __init__(self):
        ...
        self.q_target = Network(input_dimension=2, output_dimension=3)
    ...
    def calculate_Bellman_reward(self, minibatch):
        ...
        gamma = torch.ones(len(minibatch)) * 0.95
