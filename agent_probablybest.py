...
class Agent:
    ...
    def get_next_action(self, state):
        ...
        if self.number_of_episode > 20:
            self.epsilon = self.epsilon/(self.number_of_episode-20)
    ...
    def set_next_state_and_distance(self, next_state, distance_to_goal):
        ...
        if distance_to_goal < 0.03:
            print("The number of steps taken are:", self.num_steps_taken%self.episode_length)

    # Function to get the greedy action for a particular state
    def get_greedy_action(self, state):
        input = torch.tensor(state, dtype=torch.float32).unsqueeze(0)
        predicted_q_values = self.dqn.q_network.forward(input)
        predicted_q_values = predicted_q_values.detach().numpy()
        action = np.argmax(predicted_q_values)
...
class DQN:
    ...
    def calculate_Bellman_reward(self, minibatch):
        ...
        #         r = torch.tensor(i[2] - 5, dtype=torch.float32)
        #     else:
