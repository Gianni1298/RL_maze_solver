...
class Agent:
    ...
    def __init__(self):
        ...
        #Initialise minimum_distance_to_goal
        self.minimum_distance_to_goal = 4
        #evaluate_greedy_policy boolean
    ...
    def get_next_action(self, state):
        if self.num_steps_taken % self.episode_length == 0 and self.K % 5 == 0:
            self.evaluate_greedy_policy = True
            self.epsilon = 0


        state_tensor = torch.tensor(state)
        predicted_q_values = self.dqn.q_network.forward(state_tensor)
        predicted_q_values = predicted_q_values.detach().numpy()
        action = np.argmax(predicted_q_values)


        action_probability = np.ones(4) * (self.epsilon / 4)
        action_probability[action] += (1 - self.epsilon)

        action = random.choices([0,1,2,3], action_probability)
        action = action[0]
    ...
    def set_next_state_and_distance(self, next_state, distance_to_goal):
        ...
        if self.num_steps_taken % self.episode_length == self.episode_length-800 and self.K % 5 == 0:
            self.epsilon = self.epsilon_reset
            self.minimum_distance_to_goal = distance_to_goal
            print(self.minimum_distance_to_goal)
            self.evaluate_greedy_policy = False

        if self.evaluate_greedy_policy == False:
            if distance_to_goal > self.minimum_distance_to_goal + 0.2:
                self.epsilon = 0.4
            else:
                self.epsilon = self.epsilon_reset

        if self.evaluate_greedy_policy == True and self.minimum_distance_to_goal < 0.03:
...
class DQN:
    ...
    def __init__(self):
        ...
        self.q_target = Network(input_dimension=2, output_dimension=4)
